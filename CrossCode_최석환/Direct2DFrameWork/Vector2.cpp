...
{
	x = cos(Math::ToRadian(degree));
	y = -sin(Math::ToRadian(degree));
}
/*******************************************************************************
## GetDegree ##
현재 벡터의 디그리 값을 구해 반환
...
float Vector2::GetDegree()
{
	...
	if (dir.y > 0)
	{
		return 360 - Math::ToDegree(acos(dir.x));
	}
	else {
		return Math::ToDegree(acos(dir.x));
	}
...
void Vector2::Normalize(Vector2 * vector)
{
	...
	size = sqrt(vector->x*vector->x + vector->y * vector->y);
